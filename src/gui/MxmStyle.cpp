/*
 * MxmStyle.cpp - the graphical style used by MXM to create a consistent
 *				  interface
 *
...
QPalette * MxmStyle::s_palette = nullptr;
		...
		[this](const QString& path)
		{
			if (auto file = QFile{path}; file.exists())
			{
				...
				TextFloat::displayMessage(
					tr("Theme updated"),
					tr("MXM theme file %1 has been reloaded.").arg(file.fileName()),
...
QPalette MxmStyle::standardPalette() const
{
...
void MxmStyle::drawComplexControl( ComplexControl control,
...
void MxmStyle::drawPrimitive( PrimitiveElement element,
...
int MxmStyle::pixelMetric( PixelMetric _metric, const QStyleOption * _option,
...
QImage MxmStyle::colorizeXpm( const char * const * xpm, const QBrush& fill ) const
{
	QImage arrowXpm( xpm );
...
void MxmStyle::hoverColors( bool sunken, bool hover, bool active, QColor& color, QColor& blend ) const
{
	if( active )
	{
		if( sunken )
